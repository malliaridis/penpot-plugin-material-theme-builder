import {
    ...
    hexFromArgb,
    Theme,
    themeFromSourceColor, TonalPalette
...
export function createThemeColors(themeName: string, sourceColor: string) {
    ...
    for (const scheme in theme.schemes) {
        const jsonScheme: JsonScheme = theme.schemes[scheme as keyof Schemes].toJSON();
        createColorScheme(themeName, scheme, jsonScheme);
    }

    // Create palette color entries
    for (const palette in theme.palettes) {
        const tonalPalette: TonalPalette = theme.palettes[palette as keyof Palettes];
...
function createColorScheme(themeName: string, name: string, jsonScheme: JsonScheme) {
    for (const colorName in jsonScheme) {
        const argbColor = jsonScheme[colorName as keyof JsonScheme];
        const color = penpot.library.local.createColor()
        color.color = hexFromArgb(argbColor);
...
function createColorPalette(themeName: string, name: string, palette: TonalPalette) {
    toneValues.forEach(tone => {
        const argbColor = palette.tone(tone);
        const color = penpot.library.local.createColor()
        color.color = hexFromArgb(argbColor);
        color.path = `${themeName}/palettes`;
        color.name = `${name}-${tone}`;
    })
}
